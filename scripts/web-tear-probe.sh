...
#   --under-glass 0|1              Phase F arming (default 1: what guihost runs)
...
#      compositor. On KWin/Wayland with no tearing-control opt-in (and guihost's
...
#      class and driver for every run -- compare it against guihost's before
#      treating any number as transferable.
#   4. The sandbox GUI is Wayland-NATIVE and its compositor has XWayland
#      DISABLED, matching the live GUI on guihost. `underglass-sandbox.sh backend`
...
# guihost does not -- and that must NOT stop the arm from RUNNING there, because
