...
**Deterministic capture:** `~/gh/ytop/target/debug/ytop --once --mode dash --json` on jojo `54 54` (was `0 0`), `cargo test --manifest-path ~/gh/ytop/Cargo.toml` 12 passed, `cargo test -p yggterm-shell --lib web_surface` 74 passed.

**Live proof (jojo 2026-08-17, GUI 3101138, daemon 3102070, ytop 3291587 :40617):**
...
- `trace/ytop-pane-topo-jojo.json` — `titlebar_switch.active top→dash`, `search-box`, `markdown` with `Daemon Cost` + `ZFS`, timeline ring; `trace/app-state-yggterm.json` — `shell.document_surfaces [{"app_name":"Ytop","pane_id":"topo","visible":true}]`, `session_view_contract_violations []`, `active_view_mode Terminal` for ytop-verify row.
