...
2. **Confirm the screenshot is FAITHFUL before trusting it.** On guihost (KDE Wayland) `server app screenshot` can fall back to `linux_webkit_snapshot`, which **cannot capture the accelerated xterm canvas** — it returns `capture_faithful:false` with a reason, and the terminal region is blank/stale. A `faithful:false` frame is a LIE about the terminal. Check the `capture_faithful` field; if false, the capture must be fixed (Spectacle path) before I claim anything — do not reason from a frame I was told is unfaithful. Faithful Wayland capture needs Spectacle + the GUI's `DBUS_SESSION_BUS_ADDRESS` (`unix:path=/run/user/$(id -u)/bus`) in the SSH env.
...
[gstack](https://github.com/garrytan/gstack) is installed globally (`~/.claude/skills/gstack`) on pi, dev, and guihost. Use these slash commands for engineering tasks:
