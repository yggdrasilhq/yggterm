...
- **Diagnose terminal/xterm symptoms with the diagnostic toolkit, not code-reading or user-eyeballing.** `.agents/skills/yggterm-diagnostics/SKILL.md` is the index of the deterministic harnesses (`mock-tui` — already codex-like, no need to clone codex — + `pipeline_integration.rs` for the daemon pipeline; `tools/xterm-harness/` for the exact vendored xterm.js client layer; extracted decision specs like `scroll_mode.rs` / `terminal_retained_replay_policy.rs`) and the live daemon/app-control probes (`server snapshot` incl. the `pty_cols/rows` squish gauge, `server app state`, `server app terminal probe-scroll` for the `viewport_force_log`, `server terminal screen`, `server trace tail`). It also documents which instruments lie (stale `viewport_y` when backgrounded; public-vs-effective viewport; the Wayland focus trap; daemon-screen-is-authoritative). Reach for the deterministic harness FIRST; confirm on daemon ground truth, not screenshots.
