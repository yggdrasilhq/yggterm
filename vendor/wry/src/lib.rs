...
struct WebViewAttributes<'a> {
  ...
  // yggterm: upstream requires Send + Sync here because WebView2/WKWebView
  // invoke protocol handlers off the main thread. WebKitGTK raises scheme
  // requests on the GTK main loop, so the gtk build keeps the `'static`-only
  // contract dioxus-desktop's Rc-bound asset handlers need (the virtualdom
  // is not Send). DO NOT widen the gtk arm to the other backends.
  #[cfg(gtk)]
...
impl<'a> WebViewBuilder<'a> {
  ...
  /// - iOS: To get the path of your assets, you can call [`CFBundle::resources_path`](https://docs.rs/core-foundation/latest/core_foundation/bundle/struct.CFBundle.html#method.resources_path). So url like `wry://assets/index.html` could get the html file in assets directory.
  // yggterm: upstream 0.56 requires Send + Sync here because WebView2/WKWebView
  // invoke protocol handlers off the main thread. WebKitGTK raises scheme
  // requests on the GTK main loop, so the gtk build keeps the 0.55
  // `'static`-only contract that dioxus-desktop's Rc-bound asset handlers
  // need (the virtualdom is not Send). DO NOT widen the gtk arm.
  ...
  #[cfg(not(gtk))]
  // yggterm: strict arm — upstream's own 0.56 signature for the other backends.
  pub fn with_custom_protocol<F>(mut self, name: String, handler: F) -> Self
  where
