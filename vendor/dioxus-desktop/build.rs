...
fn compile_ts() {
    ...
    postprocess_native_eval("./src/js/native_eval.js");
}

fn postprocess_native_eval(path: impl AsRef<Path>) {
    let path = path.as_ref();
    let mut js = fs::read_to_string(path).expect("failed to read generated js");

    let compat_prelude = "if(typeof globalThis.WeakRef===\"undefined\"){globalThis.WeakRef=class{constructor(v){this._v=v}deref(){return this._v}}}if(typeof globalThis.FinalizationRegistry===\"undefined\"){globalThis.FinalizationRegistry=class{constructor(_cb){}register(){}unregister(){}}}";
    if !js.starts_with(compat_prelude) {
        js = format!("{compat_prelude}{js}");
    }

    js = js.replace(
        "class Channel{pending;waiting;constructor(){this.pending=[],this.waiting=[]}",
        "class Channel{constructor(){this.pending=[],this.waiting=[]}",
    );
    js = js.replace(
        "class WeakDioxusChannel{inner;constructor(channel){",
        "class WeakDioxusChannel{constructor(channel){",
    );
    js = js.replace(
        "class QueryParams{id;data;constructor(id,method,data){",
        "class QueryParams{constructor(id,method,data){",
    );
    js = js.replace(
        "class NativeDioxusChannel extends DioxusChannel{rust_to_js;request_id;constructor(request_id){super();",
        "class NativeDioxusChannel extends DioxusChannel{constructor(request_id){super();",
    );
