...
> costing any workstream here — it changes what "move the render off guihost" is worth.
...
(`payload.hot_pid`), from a copy of guihost's own `perf-telemetry*.jsonl`:
...
- **The monitor**: `summarize_perf_telemetry` read every rotated generation and
  ...
  against a copy of guihost's actual corpus: **one 60 s window reads 10,156,747 of
  110,818,506 bytes — 9.2%.**
- **The chore's tree walk**: its only consumer sits behind the LLM-generation
  opt-in, and **no daemon on guihost has `YGGTERM_ENABLE_BACKGROUND_COPY_CHORE`
...
>
> The reader's own output on guihost, which independently reproduces the hand-parse
...
**`web_surface_views_visible` became load-bearing on 2026-07-27, but it is an
APPLIED-INTENT denominator, not engine proof.** Every realized view outside it has
been told it is off screen. Whether that request actually became a hidden WebKit
widget is reported separately by `web_surface_engines_hidden`,
`web_surface_engine_widgets_visible`, `web_surface_engine_widgets_mapped`, and the
two visibility-mismatch counters. Before that date an unrevealed surface reported
`visibilityState: "visible"` to
...
More importantly, a stashed view with a nonzero mismatch count is the FAILED HIDE
itself. The second plane is deliberate precisely because it can disagree with the
first; the disagreement is the defect the old single-plane instrument hid.
...
jars writing the same on-disk `cookies` file. guihost's own telemetry corroborates it:
...
Every retained `event-trace.*.jsonl` on guihost, all `category: web_surface`:
...
generous, and a slightly heavier day puts guihost back on the ten-minute clock by
design.
...
**The measured cost.** One live `WebKitWebProcess` on guihost holds **674 MB PSS**;
§8 records a single Meta surface at ~1.3 GB. So keeping background surfaces is
expensive, and the honest bound is arithmetic: guihost has 9.2 GB available against
...
Across all 15 profiles on guihost that have a cache, hashing every blob's CONTENTS
...
⚠ **The event trace is not a reliable instrument for this on guihost.** The GUI
...
- **`Comfortable` is a headroom reading, and guihost swaps.** `MemAvailable` stays
