...
**Motivating example (user):** a Codex session opened ~monthly (± random days) with *"some time has passed, can you upgrade again"* that upgrades the whole infrastructure (guihost, main, practice, …) looking into update nuances and package-registry flaws. Today the user does this by hand; the automation does it on a timer and leaves the finished session as a keep-alive in Automated Sessions.
...
- ⬜ **Increment 5 — live verify** on guihost.

## Build / verify

Implementation goes on `experimental/automations` (worktree `~/gh/yggterm--automations`). Verify live via yggui app control on guihost (now faithful on Wayland post-2.8.0): create an automation with a short test cadence, confirm it fires, opens the session, sends the prompt, and the finished session lands in Automated Sessions (not Live), with the cwd-tree node unchanged.
