...
* Checker: `scripts/check-titles.py` — per-host `find` as above, plus for each `session_id` query `~/.yggterm/session-titles.db` (`SELECT title FROM session_titles WHERE session_id=?`) to get generated copy, then compare `effective_title` = store title else generated. For Muse (TitleAuthority::Generated) expect store `None` and generated non-empty when a session has turns; for antigravity expect store present.
