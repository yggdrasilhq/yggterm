...
| `pgrep <pattern>` **without `-f`, where the pattern is 16+ characters** | **Always, and it reports ZERO rather than failing.** Without `-f`, `pgrep` matches against `comm`, which the kernel truncates to **15 characters** (`TASK_COMM_LEN`), so a longer pattern cannot match anything that exists — the count is not wrong, it is unreachable. Measured 2026-08-14: `pgrep -c drkonqi-coredump` (16 chars) returned **0** while `pgrep -cf drkonqi-coredump-launcher` returned **18**, and the 0 was one keystroke away from being published as "that crash counter is gone, delete the claim". ⚠ This is the INVERSE of every other `pgrep` row here: those match too much and are caught by looking, this matches nothing and looks like a clean negative | `pgrep -f <pattern>` (matches the full command line, no length limit) — then apply the self-match guards in the rows below. To sanity-check any zero, re-ask with a pattern under 15 chars: if the short one finds processes and the long one finds none, the length is the whole story. ⛔ **A zero from an instrument you have not proven can return non-zero is BLIND, not empty** |
