...
| `server app row-set --into` on already `live_rail depth1` leaves (`Ychrome`, `7.0`) that are already at top | **Those 3 were never detached** — moving `Ychrome×2`+`7.0` (`child 1`) under `Continue` whooped the fleet you had at top into the group. `accepted:true` again hid it; `rows --json` showed `HEAD child 1→4` with the 3 leaves now `depth2` | Leave `depth1 child1` leaves at top; only `ssh://` with `live_runtime true` + `presence row` (detached ether PTYs) are ether to `row-set --into HEAD`; verify `depth1` stays 9 |
