...
| ID | Symptom | Status |
|----|---------|--------|
| [scrollback-lost-on-session-switch](#scrollback-lost-on-session-switch) | User-scrolled scrollback collapses to live cursor when switching sessions | PArecordsALLY FIXED |
...
| [scroll-jump-on-input](#scroll-jump-on-input) | Typing in a session yanks viewport to a "particular spot" (flicker between spot and prompt); scroll-lock variant kicks user back when scrolling | PArecordsALLY FIXED 2026-05-26 — input-snap skipped when user is reading scrollback |
...
**STATUS:** PArecordsALLY FIXED — within-session-life session switch is now
...
**STATUS:** FIXED 2026-05-26 — localStorage-based persistence verified live on guihost.
...
### Verification (2026-05-26, live on guihost)
...
### Reproduction (observed live on guihost, 2026-05-25)
...
**STATUS:** PArecordsALLY FIXED 2026-05-26 (commit 6c757b1) — the "snap to
...
Live host `guihost`, active session
...
**STATUS:** OPEN — reported live 2026-05-26 on guihost.
...
### Reproduction (current variation, on guihost 2026-05-26)
...
### Reproduction
Not yet captured deterministically. Anecdotal: guihost (Wayland) under
...
empty while neighbours paint — the 2.9.63 guihost screenshot with letters
...
**STATUS:** FIXED & VERIFIED LIVE 2026-05-28 on guihost 2.7.62.

### Live proof (added after misdiagnosis)
I reopened this earlier in the same day claiming two gaps (daemon-handoff resets vt100 ring; GUI prefers daemon_pty over snapshot). Live testing on guihost proved the gaps did not matter for the user's actual concern:
...
- Test 2: codex resume of example_org via fresh shell → `base_y: 144`, conversation rendered. SIGTERM GUI, `app launch`, `app open` → `base_y: 144` (identical screenshots).
...
The earlier `example_org base_y: 0` observation was on a session that hit a stale resume-codex wiring path, not a scrollback fix gap. Running `codex resume <UUID>` in a clean shell prints the conversation (verified live). The right diagnosis would have been to investigate example_org's launch path, not refactor the retention machinery.
...
pass (fail-then-pass proven); dev/guihost live verification still owed before "fixed"
...
### Reproduction
1. Resume a Claude Code session on a remote (live-confirmed on guihost
...
- ✅ FIXED 2026-06-06 (campaign D5): GUI-side replay JS
  ...
  the sole remaining gate.) Live-verify on guihost still owed.
...
**STATUS:** MITIGATED (frame budget) — SHIPPED & VERIFIED LIVE on guihost
...
authoritative screen frame before the client snapshot. **NEEDS LIVE VERIFY on guihost**
...
### Reproduction
Live (guihost 2.8.25): foreground a working codex session, switch away and back a few
...
same day (see §Layer 3 below). **live-verified 2026-07-25 on guihost at 2.12.11**,
...
Live-caught on guihost (2026-07-11): client viewport **167x63**, `claude` PTY on dev
...
Measured on the live fleet, same tick, from one guihost (2.12.22) daemon:
...
1. **~~Which code path leaves the husk?~~ ANSWERED 2026-07-22 — a PArecordsAL
...
Not yet captured deterministically. On guihost the obvious route does NOT
...
The precondition was measured live on guihost 2026-07-23: on the return leg of a
