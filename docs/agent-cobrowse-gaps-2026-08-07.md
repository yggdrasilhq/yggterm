...
## What is actually uniform (measured 2026-08-07 ~05:15 IST, dev and guihost)
...
touching guihost", and the co-browse doctrine says to prefer dev because **guihost is the laptop the
...
today means **guihost**. So the one class of task with the strongest reason to stay off the
...
2. **2026-08-07** — an India Post Click-n-Book booking was driven entirely on `ctl` (login, the
   ...
   a separate agent on guihost at 05:00 for a Rs 23 charge, on the machine the operator was using.
