...
display title. A bookmark created under `practice:/home/user` and renamed to
`git/samplers` represents `practice:/home/user/git/samplers`. Startpage
...
- A scoped Startpage's recent-work list is scoped by both machine and cwd. A
  local folder such as `/home/user` may show only local stored/live sessions under
