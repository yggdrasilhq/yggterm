...
- **Does NOT commit memory into the git repository:** Per `feedback-no-session-data-in-repo`, all memory files live exclusively in user-space (`~/.yggterm/memory/` and `~/.claude/...`) and travel between fleet hosts (`jojo`, `dev`, `oc`) over SSH.
...
| `sync-fleet` | Multi-host mesh synchronization across `jojo`, `dev`, `oc` over SSH | Network |
...
Unified memory syncs across the three fleet hosts (`jojo`, `dev`, `oc`) following the established fleet protocol (`reference-fleet-memory-sync.md`):
