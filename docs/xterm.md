...
## Guihost Finding, 2026-05-12

The live guihost 2.2.33 viewport showed a current Codex prompt but old status
...
In 2.6.22, the live guihost scroll incident tightened the retained-replay rule:
...
In 2.6.65, guihost reproduced the same split on the DOM renderer during
...
In 2.6.66, guihost reproduced the harder DOM-renderer failure: xterm's buffer still
...
## Guihost Finding, 2026-05-17

Wayland xterm canvas is no longer a default product renderer. A live 2.6.35
guihost client exposed a split where app-control could read daemon-backed xterm
...
## Guihost Finding, 2026-05-13

The live 2.4.0 guihost viewport could become visually blank across the upper xterm
...
## Guihost Finding, 2026-05-09

The active guihost session was visually dark, used the xterm canvas renderer, and
...
runtimes was insufficient: guihost recreated the local bridge, but dev still had
...
guihost proof moved both sessions to the current daemon, but the old 2.2.6 daemon
...
guihost live incident showed that a broad shell render loop was one bug, but even
...
In 2.6.9, the guihost fast-typing incident proved that both the Rust write bridge
...
In 2.6.38, the guihost 2.6.37 launch proof exposed a retained-recovery retry
...
In 2.2.37, the guihost diagonal line-stack failure was traced to PTY line
...
- guihost's observed `49m` bottom-row clear payload
