...
— in the ychrome browser viewport, which is a yggterm web surface, on jojo
...
**Environment parity, checked rather than assumed.** dev and jojo both run
...
### jojo (AMD `radeonsi`, the user's own GPU, isolated headless sway) — 250 frames
...
|---|---|---|---|---|---|
| `jojo-jsscroll` | 50 | 50 | **0** | 2 | 0 |
| `jojo-anim3d` | 50 | 50 | **0** | 2 | 0 |
| `jojo-anim2d` | 50 | 50 | **0** | 2 | 0 |
| `jojo-glass-jsscroll` (under-glass **1**) | 50 | 50 | **0** | 2 | 0 |
| `jojo-heavy-jsscroll` | 50 | 50 | **0** | 2 | 0 |
...
Not argued from defaults — read off jojo:
...
case the tear measurement is not sensitive to it: `jojo-glass-jsscroll` measured
under-glass `=1` on jojo's own GPU and found nothing, as did every `noglass-*`
arm on dev.
...
1. **Judder / dropped frames / uneven frame pacing during scroll and animation.**
   ...
   advancing in irregular jumps looks like it is ripping. jojo is a laptop with
...
- The sandbox compositor is **sway (headless)**, not KWin on a real KMS output.
  ...
  jojo arms remove the GPU and driver from the difference list; they do not
...
# on a GUI host with no numpy/PIL (jojo): capture there, decode elsewhere
YGGTERM_HEADLESS_BIN=~/.local/bin/yggterm YGGTERM_GUI_CLI=~/.local/bin/yggterm \
  ./scripts/web-tear-probe.sh run --label jojo-jsscroll --mode jsscroll --under-glass 0
rsync -a jojo:~/.tmp/yggterm-tear/ /tmp/yggterm-tear/jojo/
python3 tools/tear-probe/analyze.py /tmp/yggterm-tear/jojo/jojo-jsscroll --band 4
```
