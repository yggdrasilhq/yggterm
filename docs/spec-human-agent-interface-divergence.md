...
**Last verified:** daemon 3.1.3 (build 1787033587, commit 9125ff0a6dd4), `server snapshot`/`app state`/`app rows`/`terminal tenants`/`app screenshot capture_faithful:true` on 2026-08-18 — `active_terminal_surface.problem: null` (dom_paint_hit_test.problem ''), `snapshot live_sessions 27→54` `daemon owned`, paint probe `rows_in_stack && host_in_stack` ancestor fix deployed, `server app terminal adopt --target-pid` (also --outer-pid/--pid) with `app_control_pid_flag` adopt-aware, `server app open` switches viewport faithfully, **BUT** adopted row `local://b445b5bc-7e57-41db-8176-cc111a26ad94` "adopted-muse-pts29" was a plain `Shell` host (`pi@…:~$` only) — `capture_faithful:true` + `problem None` were *adjacent* answers, not history: the ether yggterm rows are `cc-runtime://…`/`codex-runtime://…` (`claude_code`/`codex` with `Queue live…`/`Resume daemon-owned…` history), and `reptyr -T <muse-pid>` is blocked (`PR_SET_DUMPABLE 0`, `ptrace_scope` even at 0 → `Operation not permitted`), so the verb must report `adopt_refused` with transplant verification (`snapshot` `terminal_lines`/`read-buffer` `text` must contain prior turns), never claim a plain shell as the ether row (see `agent-field-guide.md` one-shape row for `adopt`)  
...
*Raw `terminal_observe.rs`* had `visible_text.len<40 && contains pi@***` but `visible_text` is `"\n"`-joined dedup via raw equality (`pi@***` vs `pi@*** ` distinct → 45 chars), so the exemption never fired.  
*Fix retained in 3.1.3:* also check `buffer_text_sample|cursor_line_text|text_tail` and any `pi@***` line `<50` chars; `app state surface.problem` is now `null` while raw host field still shows the probe. **Read `active_terminal_surface.problem`, not the raw host field.**
