...
> **STATUS (2026-07-22): 4.0–4.3 SHIPPED + live (guihost 2.12.2). 4.4 (the oc WPE
...
- **★ DECIDED: shape (A) (user, 2026-07-22).** The profile write-lock IS the
  ...
  - **4.1b ✅ LANDED (main, this session).** The GUI's native web-surface
    ...
    to guihost for live proof (needs 4.1a on guihost's daemon, which predates it), best
  ...
  **4.1b/4.1c LIVE proof still owed** on guihost — a coordinated daemon+GUI deploy
  (guihost's 2.12.1 daemon predates 4.1a AND this gate fix, so PreemptedShadow is not
...
6. **Site-lore IS built and pushed — but absent on hosts that haven't pulled
   ...
   a host that hasn't `git pull`ed the ychrome repo (guihost, during this run) has
   no lore and reports it "missing" — reading that as "not built" is wrong (I
   made exactly that error by checking guihost, not dev). → Co-browse setup should
...
**Deploy state (2026-07-22, guihost v2.12.1) — CORRECTED.** The slice-2b engine
...
## Field findings — services-desk portal run (2026-07-24)
...
2. **★★ The answer to `beforeunload` decides whether a surface can ever leave its
   ...
   `beforeunload`: the services-desk portal arms one, so every SSO hand-off out of
