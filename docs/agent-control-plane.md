...
- **★ DECIDED: shape (A) (user, 2026-07-22).** The profile write-lock IS the
  ...
  **4.1b/4.1c LIVE proof still owed** — a coordinated daemon+GUI deploy to guihost
  (no protocol bump, so no version fight, but the deploy rules apply), and guihost's
