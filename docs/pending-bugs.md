...
## Landed in code, live-activation OWED on guihost
...
- **Middle-click a link in a web surface → new tab (2.10.16, e408acd8).** Root
  ...
  native-surface-blind and app-control clicks never reach a child webview). guihost
  was pinned by a concurrent agent campaign when this landed, so no GUI restart.
  When guihost is free: deploy 2.10.16, open a web surface, middle-click a link, and
