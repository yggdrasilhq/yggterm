...
- **★ SIDEBAR CWD-TREE, three user-reported bugs (2026-07-23) — ALL THREE
  ...
  2. **Local cwd-tree folders now own a REAL directory path (the remote
     ...
     Live proof on guihost: the user's one real legacy local folder healed
  ...
  3. **Slash decomposition — works via the same move.** Live proof: rename
     ...
     best-effort) — live-proven guihost→dev. Also swept 20 empty legacy
     `~/folder-<nanos>` litter dirs on guihost (one non-empty survivor left:
...
- **libyggterm apps over a MANUAL ssh hop say "not inside yggterm"
  ...
  1. **Detection — SHIPPED in code:** the daemon now also exports
     ...
     2.12.7 daemon on guihost predates it; GUI-only deploys don't activate it).
