...
`session_cwd` was **`/home/user/data/nimbusgraph`** — the ACTIVE session's cwd, not the
...
Live-proved on guihost: a rail span computes `-webkit-user-select:text` and a Range
...
Measured by the nimbusgraph row, 2026-08-08, which wrote the bug and only caught it
...
- ⛔ **Live proof on guihost** per the falsifier below — `command -v` read on the
...
Filed from atlasgraph row 5.5 after *"close the fake Google payments profile"* was driven to the
