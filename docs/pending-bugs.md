...
- **★★★ ROOT-CAUSED + FIXED 2026-07-23 — "the viewport blinks and stops taking
  ...
  **Cause.** `spawn_progressive_session_migration` (daemon.rs) is the retiring-
  ...
  reachable"* — not *"is it a SUCCESSOR"*. guihost ran three daemons: the live
...
- **★ A GUI RESTART CAN LEAVE APP CONTROL PERMANENTLY UNREACHABLE — the client
  instance record vanishes after a SUCCESSFUL register (guihost, 2026-07-22).**
  ...
  **Evidence, not inference.** The trace shows a normal, successful
  ...
  record path under `client-instances/unix--home-user--yggterm-server-2-12-3-sock/`)
  ...
  **★ RECOVERY WITHOUT ANOTHER RESTART (verified — this is the valuable half).**
  ...
  the live GUI pid:
  ```bash
  D=~/.yggterm/client-instances/unix--home-user--yggterm-server-<VER>-sock
  ...
  "executable_path":"/home/user/.local/bin/yggterm","display":":1",
...
- **B4 ROOT CAUSE FOUND (guihost, 2026-07-22): the cold-restore refusal is
  ...
  **The fail-safe arm is LIVE-PROVEN on the 2.12.4→2.12.5 guihost swap:** the
