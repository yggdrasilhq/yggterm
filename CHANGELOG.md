...
- **A row quietly running a background agent no longer looks like one you left half-typed.** The CLI draws a dim line in its *empty* composer describing the task, plus a `← 1 agent` footer, and on a screen reading that is indistinguishable from a message somebody typed and never sent — the misread cost real time, and two probes agreed with it (a lone Enter correctly does nothing on an empty composer, so it looked stuck). `gate-screen` now reports the signature outright, so a reader can tell chrome from a draft without guessing. The write guard was never fooled and is unchanged: it decides on whether the first visible character was drawn dim, which is the actual difference rather than a proxy for it.
