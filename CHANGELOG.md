...
- **The terminal can now say whether a session actually got painted, instead of only that it started to.** Opening a session builds a fresh, empty canvas and then fills it, and until now every instrument in the app stopped at the boundary between the two — "the terminal mounted" and "your text is on screen" were the same event to all of them. That is exactly where the reported ghosting and half-drawn screens live: nothing has to fail for a viewport to be blank, because it *starts* blank. Three new records per session-open close it — the moment the blank surface exists, the moment the first characters reach it (split into accepted, parsed, drawn), and a check a second later asking how many of the rows this terminal holds text on have actually been drawn. **A count of frames could never answer that**, because the renderer only redraws the rows it thinks changed, so a screen showing two lines out of fifty has the same frame count as a healthy one. Measured on a test rig: the wait between an empty canvas and the first characters is **97% waiting for the session to say something and 3% drawing it**, three costs that used to be one number. `scripts/paint-chain.py` prints the whole chain, one line per open. **The probe was wrong twice before it shipped, both times in the flattering direction** — it first timed the canvas painting itself *empty* and called that "the text arrived", and it then counted those empty repaints as rows drawn, which would have hidden precisely the half-drawn screens it exists to find. It costs 1.6% of the diagnostic log, and it is bounded by how often sessions open rather than by how much they print.
- **One session running out of credits stopped deciding whether every other session could be woken.** When a session is refused on quota the watchdog stands the whole fleet down, and for a timed limit that is right: the limit belongs to the account, so waking anything else only walks it into the same wall. An exhausted **credit balance** is not a timed limit — the refusal itself offers to switch model, which is a per-session choice, and no amount of waiting restores a balance. So the half-hour hold expired, the next pass tried again, the same wall answered, and the blackout re-armed, with **23 sessions unwakeable behind one session's billing state**. A balance refusal now suspends only the session it belongs to and tells you once, because unlike a quota window this is something you can actually fix. Wording that has not been measured is still treated as a timed limit, so nothing else changes.

- **A session nothing could wake no longer prints as a healthy one.** A refused wake is given back, so the attempt counter stays at zero — and the one line people actually read showed a session refused on every single pass as identical to a session that had simply had no reason to be woken. One went ~7.8 hours that way. The refusal, its reason and how long it has stood now sit on that session's own line, along with whether it has been escalated for never waking, because *the wake was delivered* and *the session woke up* are different facts.
