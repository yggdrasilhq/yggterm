...
- **yggtopo Dash now shows the fleet on Yggdrasil hosts.** On `***` the relay `seat-membership.json` was `{}` so Dash reported `0 live` while the daemon held 53 `live_sessions`. It now falls back to the daemon ground truth (`server snapshot` per `yggterm-diagnostics`, 54/54 on ***) and renders the 5-min Ring sparkline `▁▂▃▄▅▆▇█` (1s bucket, `Last 60s` table) — proof bundle `artifacts/demos/unreleased/ytop-dash-fallback-55e374a` with `--backend os` screenshots. Companion `yggtopo` commits `55e374a`/`017ef16` (Top `Daemon Cost` 4.5× sharing win + ZFS IOSTAT live).
