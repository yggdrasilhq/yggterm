...
`pending_media_capture` stays `null` while the page hangs — measured on guihost
...
ssh "$LIVE_HOST" "~/.local/bin/yggterm server ledger --scope gui:guihost"     # one GUI's ledger
ssh "$LIVE_HOST" "~/.local/bin/yggterm server reorder --stdin --scope gui:guihost" < /tmp/order.bak
```
