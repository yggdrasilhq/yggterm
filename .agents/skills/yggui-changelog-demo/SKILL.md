...
- Codex model-permission setup selectors are interactive terminal surfaces even when they sit mid-screen with many blank rows below the hidden cursor, no attach-ready visual deadline, or only the lower half of the selector visible to the live health tail. A proof should show `terminal_settled_kind == "interactive"`, no remote-attention notification, and `terminal_hosts[].input_enabled == true` while the selector text is visible.
