...
- For daemon hot-update, multi-version control, hung-session, or latency incident claims, start the proof with `yggterm-mock-cli --scenario panic-report --expect-path <session-path> --jsonl-out <path>`. Include `server-list`, the matching `hot-restart` result when lifecycle recovery is used, and a post-restart `latency-check --all` or `wait-session` proof so the bundle shows both the incident picture and the recovered server surface.
