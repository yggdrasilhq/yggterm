...
- The renderer contract now accepts either `dom` or `canvas`. Do not fail a proof just because xterm fell back to the DOM renderer; fail it only when the mounted host lacks visible prompt text, sane surface geometry, or cursor evidence.
...
- Do not trust the `probe-type` response by itself. Always pair it with a follow-up `server app state` and `server app screenshot`, then judge the bug from the resulting screenshot plus `terminal_hosts[].text_sample` and `terminal_hosts[].text_tail`.
