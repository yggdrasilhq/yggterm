...
- `server app terminal probe-type` and `probe-scroll` now exercise the active xterm host in the main viewport, but they are still DOM-synthetic probes. Use them to prove focus ownership, `input_enabled`, viewport movement, and host selection. For trusted keystroke proof such as `/status<Enter>` or real Tab behavior, use a second X11 display plus `xdotool`/`x11automation` against that window and then capture `server app state` and a screenshot.
