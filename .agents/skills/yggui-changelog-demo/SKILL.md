...
- For UI-theme or terminal-theme claims, prefer `server app theme light|dark --pid <pid>` over click-based toggles during proof capture. The resulting app state now exposes `settings.theme`, `settings.effective_terminal_theme_name`, and the mounted xterm renderer fields `terminal_hosts[].xterm_font_family`, `xterm_font_weight`, `xterm_font_weight_bold`, `xterm_theme_background`, and `xterm_theme_foreground`. Use those plus the screenshot to prove the live xterm surface actually changed, not just the shell chrome.
