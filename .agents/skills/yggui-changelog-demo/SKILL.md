...
- For terminal input regressions, raise the proof bar further: use `server app terminal probe-type --mode keyboard --data '/status' --enter` on a fresh second-X11 client and require the resulting screenshot plus state to show the Codex status panel, no transport-error toast, and a still-interactive terminal.
