...
- For fresh remote Codex startup over SSH, use `server app terminal new --machine-key <machine> --kind codex` or the `codex_spawn_timeline` smoke. Capture sub-1s, 1s, 2s, 5s, ready, and post-30s states/screenshots, then reject the proof if the session-specific `Remote Terminal Needs Attention` notification appears, if `terminal_settled_kind` is not `interactive` after settle, if `terminal_hosts[].input_enabled` is not true, or if the active surface problem remains set.
