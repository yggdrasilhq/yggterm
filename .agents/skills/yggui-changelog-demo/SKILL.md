...
- For terminal input regressions, raise the proof bar further: use `server app terminal probe-type --mode keyboard --data '/status' --enter` on a fresh second-X11 client and require the resulting screenshot plus state to show `/status` in the prompt area, the Codex status panel, no transport-error toast, a visible cursor at the next prompt, and a still-interactive terminal.
...
- Cursor visibility now has explicit app-control evidence too: `terminal_hosts[].cursor_overlay_present`, `cursor_overlay_display`, and `cursor_overlay_rect`. For light-theme terminal readability fixes, reject the proof unless the screenshot itself shows the cursor and the state confirms the overlay is mounted and visible.
- Cursor alignment now has explicit app-control evidence too: compare `terminal_hosts[].cursor_overlay_rect` against `cursor_expected_rect`, and use `cursor_sample_rect` as supporting evidence when xterm emits a visible raw cursor span. Reject the fix if the overlay floats away from the expected cursor cell.
- When xterm emits a very wide raw `.xterm-cursor` span, do not fail on width alone. Fail only if that wide span is still visually active via background, border, outline, or box-shadow. The overlay cursor remains the primary visible cursor contract.
