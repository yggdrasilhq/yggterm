...
Built and live-proven on guihost (yggterm `010b9957`, ychrome `8452654`). ychrome's
...
`doubleclick.net`, `googlesyndication.com` and ~28 other names in guihost's ruleset
...
Check with `getent hosts <domain>` first. On guihost only two blocked domains
...
- **An unanswered `permission-request` is NOT a deny.** Measured on guihost before
