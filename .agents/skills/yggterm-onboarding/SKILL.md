...
(***/dev/oc) — each harness and each host starts isolated. Critical bug findings,
...
Claude learned on `***` yesterday, without anyone hand-copying files.
...
- **No fleet drift.** Without it, `~/.claude/projects/<ns>/memory` on *** and dev
...
2. **Wire auto-sync so they never have to remember it** (do it *with* them, show the timers):
   ...
   #   ygg-memory sync-harness --all + ygg-memory sync-fleet --mesh "*** dev oc"
   ...
   why *** stops running hot when work is on dev/oc.
