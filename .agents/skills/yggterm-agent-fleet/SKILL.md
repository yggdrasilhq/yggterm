...
| **Main-move aware** | the build trigger is `integration HEAD != last attempted build sha` (2026-09-13, dream ACK-180c05f734): an external main push — a seat landing directly, a hotfix, a human — is fetched, built, gated and deployed on the next tick with no subscription needed (subs are still what put a project under the plane at all). The baseline counts failed attempts, so a red main never rebuild-loops, and a failed sub-integration resetting to bare main re-arms the bare-main build (build-the-REST). Before this, only sub-driven movement built: a direct main push sat unbuilt until some unrelated lane landed. |
