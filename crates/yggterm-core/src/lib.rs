...
pub use transcript::{
    ...
    transcript_messages_from_entries, transcript_view_messages,
};
pub use workspace::{
...
///
/// Live-evidenced on guihost 2026-07-11: `20e56a8b-…` was BORN in
/// `/home/user/gh/yggterm` (spawn cwd correct, transcript written under
/// `-home-user-gh-yggterm`), but every relaunch from the start page spawned in
/// `/home/user`, so CC looked in `-home-pi`, found nothing, and refused. The
...
/// (`/home/user/git/x.y` -> `-home-user-git-x-y`): every character outside
...
pub fn read_cc_session_identity_fields(path: &Path) -> Result<Option<(String, String)>> {
    ...
    // Live case (guihost 2026-07-23): a session launched in /home/user (the old
    // folder-cwd bug), /cd'd to its real project — 307 records say /home/user,
    // the last 4 say the real path, and the FILE lives under the real path's
    // encoding. First-cwd bucketed it under /home/user (invisible among 647
...
mod tests {
    ...
    ///
    /// Regression lock for the guihost 2026-07-11 "No conversation found with session ID"
    /// bug: `20e56a8b-…` was born in `/home/user/gh/yggterm` but every relaunch spawned
    /// `claude --resume` in `/home/user`, so CC resolved its project dir to `-home-pi`,
    ...
    /// the wrong answer for both grouping and resume. Live case (guihost
    /// 2026-07-23): a session launched in /home/user (the old folder-cwd bug),
    /// /cd'd to its real project — 307 records said /home/user, the last 4 said
    /// the real path, and the file lived under the real path's encoding; the
    /// session bucketed under /home/user, invisible among 647 rows, and its
    ...
    fn cc_session_cwd_follows_cd_rehoming_not_the_first_record() {
        ...
        fs::write(
            ...
            format!(
                ...
                record("/home/user"),
                record("/home/user"),
                record("/home/user/git/exam-prep-notes"),
            ),
        )
        .expect("write transcript");
        assert_eq!(
    ...
    fn cc_session_cwd_falls_back_to_a_non_user_record_rather_than_vanishing() {
        ...
        let projects = root.join("projects");
        let project = projects.join("-home-user-git-charts");
        fs::create_dir_all(&project).expect("create project dir");
        ...
        fs::write(
            ...
            format!(
                "{}\n{}\n",
                serde_json::json!({"type":"assistant","sessionId":session_id,"cwd":"/home/user/git/charts"}),
        ...
        fs::write(
            ...
            format!(
                ...
                serde_json::json!({"type":"user","cwd":"/home/user/git/charts"}),
            ),
        )
        .expect("rewrite transcript");
        assert_eq!(
    ...
    fn workspace_documents_sort_before_raw_codex_sessions_inside_project_folder() {
        ...
        insert_workspace_document(
            ...
            &WorkspaceDocumentSummary {
                ...
                virtual_path: "/home/user/0000-local-shell".to_string(),
        ...
        insert_codex_browser_project(
            ...
            &LocalAgentProjectBucket {
                ...
                sessions: vec![LocalAgentSessionSummary {
                    kind: SessionKind::Codex,
                    file_path: PathBuf::from("/home/user/.codex/sessions/example.jsonl"),
        ...
        browser.ensure_visible_path("/home/user/0000-local-shell");
        let rows = browser.rows();
        let recipe_ix = rows
            .iter()
            .position(|row| row.full_path == "/home/user/0000-local-shell")
            .expect("recipe row should be visible");
        let session_ix = rows
            .iter()
            .position(|row| row.full_path == "/home/user/.codex/sessions/example.jsonl")
