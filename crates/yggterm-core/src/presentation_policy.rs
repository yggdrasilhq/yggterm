        ...
        PresentationTarget::LinuxWayland => &[
            ...
            SanctionedVar {
                ...
                why: "HARDWARE VIDEO DECODE. WebKitGTK decodes through GStreamer, which loads \
                      ...
                      Measured on guihost 2026-08-01 with a YouTube video playing: libgstva.so and \
...
mod tests {
    ...
    /// The live arming measured on guihost, 2026-08-01, from the startup trace.
    fn guihost_live() -> Vec<(&'static str, &'static str)> {
        vec![
            ("GDK_BACKEND", "wayland"),
    ...
    fn the_live_host_matches_the_wayland_row_once_the_decoder_rank_is_set() {
        ...
        let deviations = deviations(PresentationTarget::LinuxWayland, &env_of(&guihost_live()));
        assert!(
            deviations.is_empty(),
