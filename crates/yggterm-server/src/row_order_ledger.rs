...
//! client identity string (e.g. `gui:guihost:/home/user/.yggterm`); the daemon
...
mod tests {
    ...
    fn per_scope_orders_are_independent_with_shared_fallback() {
        let mut ledger = RowOrderLedger::default();
        ledger.record_live_order("shared", &rows(&["a", "b", "c"]));
        ledger.record_live_order("gui:guihost", &rows(&["c", "b", "a"]));
        assert_eq!(ledger.scope_rows("gui:guihost"), rows(&["c", "b", "a"]));
        ...
        let is_live = |row: &str| live.iter().any(|entry| entry == row);
        assert_eq!(
            ledger.placement_for("gui:guihost", "d", is_live),
    ...
    fn save_load_round_trip() {
        ...
        let mut ledger = RowOrderLedger::default();
        ledger.record_live_order("gui:guihost", &rows(&["a", "b"]));
        ledger.save(&dir).expect("save");
        let loaded = RowOrderLedger::load(&dir);
        assert_eq!(loaded.scope_rows("gui:guihost"), rows(&["a", "b"]));
        let _ = std::fs::remove_dir_all(&dir);
    }
}
