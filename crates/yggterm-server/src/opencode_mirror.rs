        ...
        for (key, arg) in candidates {
            ...
            if known {
                if arg == key_id {
                    ...
                    //
                    // ⛔ ALREADY-BOUND IS NOT A REFUSAL (2026-09-03): the
                    // mirror-tick rebind (sibling lane) moves a live anchor's
                    // id to the viewed session AHEAD of this pass in the same
                    // tick — by the time this probe runs, id already equals
                    // arg and the applicator (which returns true only when the
                    // row MOVED) answers false. Reading that as "refused"
                    // would stamp a healthy row with a failure verdict every
                    // tick; the row is BOUND, which is the converged state.
                    let already_bound = self
                        .resolve_session_storage_key(&key)
                        .and_then(|resolved| self.sessions.get(resolved))
                        .is_some_and(|session| session.id == arg);
                    if already_bound {
                        verdict = format!("bound to {arg}");
                    } else if self.apply_agent_runtime_session_id_to_live_session(&key, &arg) {
                        let resolved = self
                            .resolve_session_storage_key(&key)
                            .map(str::to_string);
