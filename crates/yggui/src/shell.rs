...
mod tests {
    ...
    fn merged_sidebar_rows_compress_single_child_remote_folder_chains() {
        ...
        assert!(rows.iter().any(|row| {
            row.kind == BrowserRowKind::Group
                && row.full_path == "__remote_folder__/guihost/mnt/net/data/netshare/dada/obsidian/codex"
                && row.label == "/mnt/net/data/netshare/dada/obsidian/codex"
        }));
        assert!(!rows.iter().any(|row| row.full_path == "__remote_folder__/guihost/run"));
    }
    ...
    fn merged_sidebar_rows_break_compression_when_middle_folder_has_sessions() {
        ...
        assert!(!rows.iter().any(|row| row.full_path == "__remote_folder__/guihost/home"));
    }
