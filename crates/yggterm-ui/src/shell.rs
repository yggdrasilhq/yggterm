    ...
    if target.is_empty() {
        state.with_mut(|shell| {
            shell.push_notification(
                NotificationTone::Warning,
                "SSH Target Needed",
                "Enter an SSH target such as dev, pi@guihost, or user@ip.",
            );
        });
        return;
    }
        ...
        div {
            ...
            div {
                ...
                input {
                    ...
                    placeholder: "dev or pi@guihost or user@192.0.2.15",
                    style: format!(
...
mod tests {
    ...
    fn drop_target_rejects_self_and_descendants() {
        ...
        assert!(!valid_drop_target(&["/home/user/gh/notes/folder-a".to_string()], &target));
        assert!(!valid_drop_target(&["/home/user/gh/notes/folder-a/sub".to_string()], &target));
        assert!(valid_drop_target(&["/home/user/gh/notes/other-paper".to_string()], &target));
    }
}
